...
# # get handle - An error occurred evaluating bleu: an integer is required
# def evaluate_bleu(results):
#     """ Evaluate the fixes generated by the models using the CodeBLEU metric """
#     log.info(dash_line)
#     log.info("Calculating BLEU scores...")
#     # try:
#     # Ensure all fixes are tokenized
#     human_baseline_fixes = [fix.split() for fix in results["human_baseline_fixes"].tolist()]
#     original_model_fixes = [fix.split() for fix in results["original_model_fixes"].tolist()]
#     instruct_model_fixes = [fix.split() for fix in results["instruct_model_fixes"].tolist()]
